              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      try {
                        // Replaced localhost with your live Render backend
                        const res = await fetch(`https://prophecyai.onrender.com/search-properties?query=${propertySearchInput}`);
                        const data = await res.json();
                        if (data && data.listings) {
                          setLiveListings(data.listings);
                        }
                      } catch (err) {
                        alert("Failed to connect to backend server. Make sure your Render backend is awake!");
                      }
                    }}
